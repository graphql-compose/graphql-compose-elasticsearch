...
export type CommonOpts<TContext = any> = {
  prefix?: string;
  postfix?: string;
  pluralFields?: string[];
  elasticIndex: string;
  ...
  getOrCreateOTC: (
    name: string,
    onCreate: () => ObjectTypeComposerAsObjectDefinition<any, TContext>
  ) => ObjectTypeComposer<any, TContext>;
  getOrCreateITC: (
    name: string,
    onCreate: () => InputTypeComposerAsObjectDefinition
  ) => InputTypeComposer<TContext>;
  getOrCreateETC: (
    name: string,
    onCreate: () => EnumTypeComposerAsObjectDefinition
    ...
    getOrCreateOTC: (typeName, cfgOrThunk) => {
      return schemaComposer.getOrSet(typeName, () => {
        const tc = schemaComposer.createObjectTC(
          isFunction(cfgOrThunk) ? (cfgOrThunk as any)() : cfgOrThunk
        );
        return tc;
      });
    },
...
export function reorderKeys<T extends Record<any, any>>(obj: T, names: string[]): T {
  const orderedFields = {} as Record<string, any>;
  const fields = { ...obj };
  names.forEach((name) => {
    if (fields[name]) {
